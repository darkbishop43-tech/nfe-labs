...
function extractFifteenMinuteLinks(html) {
  ...
  while ((m = re.exec(html))) {
    ...
    if (!/btc|bitcoin/.test(hay)) continue;
...
function parseEventPage(html, url) {
  ...
  const asset = /\bbtc\b|bitcoin/i.test(text) ? "BTC" : null;
  const title = text.match(/BTC 15 min[^$]{0,80}/i)?.[0]?.trim() || null;
...
async function discover() {
  const indexRes = await fetch(RH_CRYPTO_URL, {
    headers: { accept: "text/html", "user-agent": "NFE-Market-Edge-15m-Observer/1.1" },
    cf: { cacheTtl: 0 },
  });
  ...
  for (const link of links.slice(0, 12)) {
    try {
      ...
      if (row.asset === "BTC" && row.title) rows.push(row);
    } catch {}
  }
  ...
  return {
    ok: true,
    experiment: "MARKET EDGE — BTC 15 MIN REAL OBSERVER",
    mode: "READ_ONLY",
    ...
    liveCoverage: { BTC: btc.length },
    events: btc,
    caveat: "Public-page observation only. Existing Market Edge scoring/thresholds are unchanged. Contract settlement uses the event benchmark, not Coinbase spot.",
    observedAt: new Date().toISOString(),
  };
}

function page() {
  return `<!doctype html><meta name="viewport" content="width=device-width,initial-scale=1"><title>Market Edge — BTC 15 Min Observer</title>
...
<div class="wrap"><h1>Market Edge — BTC 15 Min Real Observer</h1><div class="card"><b class="good">BTC 15 MIN ONLY · OBSERVE ONLY</b><p>Same Market Edge strategy. Different instrument window: live BTC 15-minute event contracts, with the existing 5-minute maximum hold horizon.</p><small>No strategy, threshold, bankroll, or execution-rule changes. Existing experiments remain untouched.</small></div><div id="s" class="card">Loading live BTC 15-minute observation…</div><div id="g" class="grid"></div></div>
<script>
async function load(){const s=document.getElementById('s'),g=document.getElementById('g');try{const r=await fetch('/proof',{cache:'no-store'}),d=await r.json();s.innerHTML='<b>Live BTC 15-minute coverage:</b> '+d.liveCoverage.BTC+'<br><small>Observed '+new Date(d.observedAt).toLocaleString()+'</small>';g.innerHTML=(d.events||[]).map(x=>'<div class="event"><div class="big">BTC · 15 MIN</div><div>'+String(x.title||'')+'</div><p>Target: '+(x.target?'$'+x.target.toLocaleString():'—')+'<br>Bid: '+(x.bidCents??'—')+'¢ · Ask: '+(x.askCents??'—')+'¢</p><b class="good">LIVE · OBSERVE ONLY</b><br><a href="'+x.url+'" target="_blank">View Robinhood event</a></div>').join('')||'<div class="card warn">No live BTC 15-minute contract was discoverable on this refresh. No other crypto or longer-duration contract will be substituted.</div>'}catch(e){s.innerHTML='<b class="warn">PUBLIC OBSERVATION UNAVAILABLE</b><br>No trade action permitted.'}}</script><script>load();setInterval(load,15000)</script>`;
}
...
export default {
  async fetch(request) {
    ...
    if (url.pathname === "/health") return json({ ok:true, service:"market-edge-btc-15m-observer", mode:"READ_ONLY", strategyChanged:false, orderSubmission:"DISABLED" });
