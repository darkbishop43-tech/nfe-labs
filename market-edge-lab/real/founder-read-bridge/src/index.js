...
const ALLOWED=new Set(["/shadow-state","/wide-radar-state","/execution-test-state","/real-trade-state","/founder-manual-preview"]);
...
export default{async fetch(request,env){
  ...
  const r=await env.BASELINE.fetch(new Request(UPSTREAM+u.pathname+u.search,{method:"GET",headers:{"accept":"application/json"}}));
