...
// Founder Manual Close V1 is PREVIEW-ONLY. No provider write function is reachable from
// these helpers or routes. A future separately-authorized submit lane must reconcile again.
function founderManualRemainingCount(record){
  const entry=Number(record?.entryFillQuantity??record?.fillCount??record?.entryFilledCount??0);
  const closed=Number(record?.closeFillQuantity??record?.exitFilledTotal??0);
  return Math.max(0,entry-closed);
}
function founderManualClosePayload(record,liveOutcomeBid,clientOrderId,requestedCount){
  if(String(record?.source||"")!==FOUNDER_MANUAL_SOURCE)return{ok:false,state:"FOUNDER_CLOSE_REJECT_NOT_FOUNDER_OWNED"};
  const remaining=founderManualRemainingCount(record),requested=Number(requestedCount);
  if(!(remaining>0))return{ok:false,state:"FOUNDER_CLOSE_REJECT_ZERO_REMAINING"};
  if(!Number.isFinite(requested)||requested<=0||Math.abs(requested-remaining)>1e-9)return{ok:false,state:"FOUNDER_CLOSE_REJECT_EXACT_REMAINING_REQUIRED",remainingQuantity:remaining,requestedQuantity:Number.isFinite(requested)?requested:null};
  const outcome=String(record?.outcomeSide||record?.side||"").toUpperCase(),entrySide=kalshiV2BookSide(outcome);
  if(!entrySide)return{ok:false,state:"FOUNDER_CLOSE_REJECT_INVALID_OUTCOME"};
  const bid=Number(liveOutcomeBid),yesLegPrice=outcome==="YES"?bid:1-bid;
  if(!Number.isFinite(bid)||bid<=0||bid>=1||!Number.isFinite(yesLegPrice)||yesLegPrice<=0||yesLegPrice>=1)return{ok:false,state:"FOUNDER_CLOSE_REJECT_INVALID_QUOTE"};
  const body={ticker:String(record?.ticker||record?.marketTicker||""),client_order_id:String(clientOrderId),side:entrySide==="bid"?"ask":"bid",count:remaining.toFixed(2),price:yesLegPrice.toFixed(4),time_in_force:"immediate_or_cancel",self_trade_prevention_type:"taker_at_cross",post_only:false,cancel_order_on_pause:true,reduce_only:true};
  if(!body.ticker)return{ok:false,state:"FOUNDER_CLOSE_REJECT_INVALID_TICKER"};
  return{ok:true,state:"FOUNDER_CLOSE_PREVIEW_READY_LOCKED",remainingQuantity:remaining,body};
}
function founderManualCloseDryRun(record,{requestedCount,liveOutcomeBid,quoteAgeMs=0,providerQuantity,autoTickerConflict=false}={}){
  if(String(record?.source||"")!==FOUNDER_MANUAL_SOURCE)return{ok:false,state:"FOUNDER_CLOSE_REJECT_NOT_FOUNDER_OWNED",providerWrites:0};
  const remaining=founderManualRemainingCount(record);
...
async function founderManualClosePreview(env,record,requestedCount){
  if(String(record?.source||"")!==FOUNDER_MANUAL_SOURCE)return{ok:false,state:"FOUNDER_CLOSE_REJECT_NOT_FOUNDER_OWNED",providerWrites:0};
  const ticker=String(record?.ticker||record?.marketTicker||""),remaining=founderManualRemainingCount(record);
  if(!ticker||!(remaining>0))return{ok:false,state:!ticker?"FOUNDER_CLOSE_REJECT_INVALID_TICKER":"FOUNDER_CLOSE_REJECT_ZERO_REMAINING",providerWrites:0};
  ...
  const liveBid=normalizeProbability(outcome==="YES"?(m?.yes_bid_dollars??m?.yes_bid):(m?.no_bid_dollars??m?.no_bid));
...
function executionTestSeriesLimit(state){
  const requested=Math.trunc(Number(state?.maxAttempts));
  return Number.isFinite(requested)&&requested>=1&&requested<=EXECUTION_TEST_CONFIG.maxSelectableAttempts
    ? requested
    : EXECUTION_TEST_CONFIG.defaultMaxAttempts;
}
function executionTestEntryAuthorized(state){
  // attemptsStarted increments immediately before the provider write latch.
  // <= allows the final latched attempt to submit; runExecutionTestSeries prevents
  // creating any attempt beyond the per-series Founder-selected limit.
  return Boolean(state?.armed && Number(state?.attemptsStarted||0)<=executionTestSeriesLimit(state));
}
...
async function runExecutionTestSeries(env,freshShadow=null,preparedBalance=null){
  ...
  for(const observed of candidates){
    if(slots<=0||Number(state.attemptsStarted||0)>=executionTestSeriesLimit(state))break;
  ...
  if(Number(state.attemptsStarted||0)>=executionTestSeriesLimit(state)){
...
<div class="w">
  ...
  <div class="card section">
    ...
    <div id="liveTradeWatch" class="liveTradeWatch idle" aria-live="polite">
      <div class="liveTradeWatchTop"><div><b>🎣 LIVE ORDER WATCH</b><div class="m">READ ONLY · execution-test position + live Kalshi bid · refreshes every 2 seconds</div></div><span id="liveTradeWatchStatus" class="tag">CHECKING…</span></div>
      <div id="liveTradeWatchGrid" class="liveTradeGrid"><div class="m">Waiting for the next fish…</div></div>
    </div>
  ...
  <div class="card section">
    <div style="display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap">
      <div><b>🧪 Execution Test · Live Positions Mirror</b><div class="m">READ ONLY · mirrors NFE-OS filled test positions and managed exits</div></div>
      <span id="testPositionSummary" class="tag">CHECKING…</span>
    </div>
    <div id="testPositionGrid" class="opps" style="margin-top:10px"><div class="m">Waiting for execution-test state…</div></div>
  </div>
...
async function load(){
  ...
  try{
    ...
    const testPositionSummary=E('testPositionSummary'),testPositionGrid=E('testPositionGrid');
    if(testPositionSummary&&testPositionGrid){
      const testPositions=Array.isArray(executionTest?.state?.positions)?executionTest.state.positions:[];
      const openTestPositions=testPositions.filter(p=>p?.status==='OPEN'||p?.status==='EXIT_RETRY');
      const attemptsStarted=Number(executionTest?.state?.attemptsStarted||0);
      testPositionSummary.textContent=(executionTest?.state?.armed?'ACTIVE · ':'')+openTestPositions.length+' OPEN · '+attemptsStarted+'/'+Number(executionTest?.state?.maxAttempts||executionTest?.safety?.maxAttempts||5)+' ATTEMPTS';
      testPositionSummary.className='tag '+(openTestPositions.length?'good':'');
      if(openTestPositions.length){
        testPositionGrid.innerHTML=openTestPositions.map(p=>{
          const fill=Number(p?.entryAverageFillPrice);
          const fee=Number(p?.entryAverageFeePaid);
          const filledAt=Number(p?.filledAt);
          ...
          return '<div class="opp onRadar"><div class="oppHead"><div><div class="q">'+esc(p?.asset||'ASSET')+' · '+esc(p?.direction||p?.side||'')+'</div><div class="meta">'+esc(p?.ticker||'')+'</div></div><div class="oppBadges"><div class="tag radar">🎣 POSITION OPEN</div><div class="scoreBadge hot"><small>ENTRY SCORE</small><strong>'+(Number.isFinite(Number(p?.entryScore))?Number(p.entryScore).toFixed(2):'—')+'</strong></div></div></div><div class="compactGrid" style="margin-top:8px"><div class="miniBox"><div class="label">Side</div><div class="miniVal">'+esc(p?.side||'—')+'</div></div><div class="miniBox"><div class="label">Filled qty</div><div class="miniVal">'+(Number.isFinite(Number(p?.filledCount))?Number(p.filledCount).toFixed(2):'—')+'</div></div><div class="miniBox"><div class="label">Avg fill</div><div class="miniVal">'+fillText+'</div></div><div class="miniBox"><div class="label">Entry fee</div><div class="miniVal">'+feeText+'</div></div><div class="miniBox"><div class="label">Open timer</div><div class="miniVal">'+ageText+'</div></div><div class="miniBox"><div class="label">Exit control</div><div class="miniVal">'+exitState+'</div><div class="miniSub">≤ .20 OR 5:00 max hold</div></div></div><div class="meta">Entry order: '+esc(p?.entryOrderId||'—')+(p?.exitOrderId?' · Exit order: '+esc(p.exitOrderId):'')+'</div></div>';
        }).join('');
      }else{
        const recentAttempts=Array.isArray(executionTest?.state?.attempts)?executionTest.state.attempts.slice().reverse():[];
        testPositionGrid.innerHTML=recentAttempts.length?recentAttempts.map(a=>'<div class="opp"><div class="oppHead"><div class="q">Attempt '+Number(a?.attemptNo||0)+' · '+esc(a?.asset||'')+' · '+esc(a?.side||'')+'</div><div class="tag">'+esc(a?.status||'UNKNOWN')+'</div></div><div class="meta">'+esc(a?.ticker||'')+' · score '+(Number.isFinite(Number(a?.liveScore))?Number(a.liveScore).toFixed(2):'—')+(a?.orderId?' · order '+esc(a.orderId):'')+'</div></div>').join(''):'<div class="opp"><div class="meta">'+(executionTest?.state?.armed?'Series armed · fishing for attempt '+(attemptsStarted+1)+' of '+Number(executionTest?.state?.maxAttempts||executionTest?.safety?.maxAttempts||5)+'.':'No execution-test position is currently open.')+'</div></div>';
      }
    }
...
async function refreshLiveTradeWatch(){
  const box=E('liveTradeWatch'),grid=E('liveTradeWatchGrid'),status=E('liveTradeWatchStatus');
  if(!box||!grid||!status)return;
  try{
    const reads=await Promise.all([
      fetch('/execution-test-state',{cache:'no-store'}),
      fetch('/kalshi-live-mirror-data',{cache:'no-store'})
    ]);
    ...
    status.textContent=(open.length?'🎣 ':'')+open.length+' OPEN · '+Number(test?.state?.attemptsStarted||0)+'/'+Number(test?.state?.maxAttempts||test?.safety?.maxAttempts||5)+' ATTEMPTS';
    status.className='tag '+(open.length?'good':'');
    box.classList.toggle('idle',open.length===0);
    grid.replaceChildren();
    if(!open.length){
      const closed=positions.filter(p=>p?.status==='CLOSED').sort((a,b)=>Number(b?.closedAt||0)-Number(a?.closedAt||0))[0];
      const msg=document.createElement('div');
      msg.className='m';
      msg.textContent=closed?'✅ Last fish closed: '+String(closed?.asset||'')+' · attempt '+Number(closed?.attemptNo||0)+' · NFE-OS is fishing again.':'No test position is open. NFE-OS is fishing.';
      grid.appendChild(msg);
      return;
    }
    for(const p of open){
      const row=rows.find(r=>String(r?.ticker||'')===String(p?.ticker||'')&&String(r?.outcomeSide||'')===String(p?.side||''));
      const bid=Number(row?.selectedBid),qty=Number(p?.filledCount),fill=Number(p?.entryAverageFillPrice),started=Number(p?.filledAt);
      const age=Number.isFinite(started)?Math.max(0,Date.now()-started):0;
      const mm=String(Math.floor(age/60000)).padStart(2,'0');
      const ss=String(Math.floor((age%60000)/1000)).padStart(2,'0');
      const card=document.createElement('div');
      card.className='liveTradeCard open';
      const title=document.createElement('div');
      title.className='liveTradeTitle';
      title.textContent='🎣 '+String(p?.asset||'ASSET')+' · '+String(p?.direction||p?.side||'')+' · POSITION OPEN';
      const sub=document.createElement('div');
      sub.className='m';
      sub.textContent=String(p?.ticker||'')+' · score '+(Number.isFinite(Number(p?.entryScore))?Number(p.entryScore).toFixed(2):'—');
      const metrics=document.createElement('div');
      metrics.className='liveTradeMetrics';
      const vals=[
        ['ENTRY AVG',Number.isFinite(fill)?(fill*100).toFixed(1)+'¢':'—'],
        ['LIVE BID',Number.isFinite(bid)?(bid*100).toFixed(1)+'¢':'—'],
        ['MARK @ BID',Number.isFinite(bid)&&Number.isFinite(qty)?'USD '+(bid*qty).toFixed(2):'—'],
        ['OPEN TIME',mm+':'+ss]
      ];
      for(const v of vals){
        const m=document.createElement('div');
        m.className='liveTradeMetric';
        const l=document.createElement('span');l.className='label';l.textContent=v[0];
        const b=document.createElement('b');b.textContent=v[1];
        m.append(l,b);metrics.appendChild(m);
      }
      const order=document.createElement('div');
      order.className='liveTradeOrder';
      order.textContent='Qty '+(Number.isFinite(qty)?qty.toFixed(2):'—')+' · AUTO MANAGED · exit ≤ .20 score OR 5:00 max hold · Entry '+String(p?.entryOrderId||'—');
      card.append(title,sub,metrics,order);
      grid.appendChild(card);
    }
  }catch(error){
    status.textContent='WATCH READ FAILED';status.className='tag warn';
    grid.textContent='Read-only watch refresh failed. Execution controller is unaffected.';
  }
}
...
load();refreshKalshiMirror();refreshLiveTradeWatch();paintDashboardCountdown();setInterval(paintDashboardCountdown,1000);setInterval(refreshPrices,10000);setInterval(refreshKalshiMirror,2000);setInterval(refreshLiveTradeWatch,2000);
</script>
<script>
  ...
  async fetch(request, env) {
    ...
    if (request.method === "GET" && url.pathname === "/micro-lifecycle-validation-control") {
      ...
      const page="<!doctype html><html><head><meta name='viewport' content='width=device-width,initial-scale=1'><title>NFE-OS Micro Lifecycle Validation</title><style>body{font-family:system-ui;background:#050a11;color:#eef7ff;padding:18px;max-width:720px;margin:auto}.box{border:1px solid #294764;background:#0d1724;border-radius:14px;padding:16px;margin:12px 0}.ok{color:#67e49b}.warn{color:#ffd86a}.lock{color:#ff8b8b}button{width:100%;padding:15px;font-size:17px;font-weight:800;border-radius:10px;border:1px solid #d3a53a;background:#182536;color:#ffd86a}code{color:#b9dcff}</style></head><body>"+
        ...
        "<div class='box'><b>FOUNDER AUTHORIZATION</b><p>Exactly one attempt. Authorization self-retires when that sole attempt is consumed. NO_FILL also consumes it.</p><form method='post' action='/micro-lifecycle-validation-control'><button type='submit'>AUTHORIZE ONE MICRO LIFECYCLE VALIDATION</button></form></div>"+
    ...
    if (request.method === "POST" && url.pathname === "/micro-lifecycle-validation-control") {
      const state=await loadExecutionTestState(env);
      const balanceProof=await kalshiExecutionBalanceSnapshot(env);
      const rows=Array.isArray(balanceProof?.body?.balance_breakdown)?balanceProof.body.balance_breakdown:[];
      const index2=Number(rows.find(x=>Number(x?.exchange_index)===2)?.balance);
      const openPositions=executionTestOpenPositions(state);
      const consumed=Boolean(state?.microLifecycleValidation?.consumed||state?.microLifecycleValidation?.authorizedAt);
      const clean=!executionTestQueueActive(state)&&state?.armed!==true&&openPositions.length===0&&Number(state?.attemptsStarted||0)>=0;
      if(consumed)return json({ok:false,status:"MICRO_LIFECYCLE_AUTHORITY_ALREADY_CONSUMED",armed:Boolean(state?.armed),authorization:false,secondAuthorizationAllowed:false},409);
      if(!clean||!balanceProof?.ok||!Number.isFinite(index2)||index2<1)return json({ok:false,status:"MICRO_LIFECYCLE_PREFLIGHT_FAILED",armed:Boolean(state?.armed),authorization:false,openPositions:openPositions.length,index2CashUsd:Number.isFinite(index2)?index2:null},409);
      const priorQueue=state.queue&&typeof state.queue==="object"?JSON.parse(JSON.stringify(state.queue)):null;
      state.microLifecycleValidation={authorizedAt:new Date().toISOString(),consumed:true,selfRetiring:true,maxAttempts:1,maxConcurrent:1,maxEntryDebitUsd:1,threshold:REAL_TEST_CONFIG.entryScore,secondAuthorizationAllowed:false,priorQueue};
      state.seriesId=crypto.randomUUID();state.armedAt=Date.now();state.completedAt=null;state.threshold=REAL_TEST_CONFIG.entryScore;state.maxAttempts=1;state.maxConcurrent=1;state.maxEntryDebitUsd=1;state.attemptsStarted=0;state.attempts=[];state.positions=[];state.ledger=[];state.armed=true;state.status="MICRO_LIFECYCLE_AUTHORIZED_FISHING";
      state.queue={active:false,status:"MICRO_VALIDATION_ONE_SHOT",queueId:null,currentIndex:-1,totalAttempts:1,completedAttempts:0,stages:[],completedStages:[],cancelledStages:[]};
      executionTestLedger(state,"MICRO_LIFECYCLE_ONE_SHOT_AUTHORIZED",{maxAttempts:1,maxConcurrent:1,maxEntryDebitUsd:1,threshold:REAL_TEST_CONFIG.entryScore,selfRetiring:true});
      await saveExecutionTestState(env,state);
      return new Response("<!doctype html><html><head><meta name='viewport' content='width=device-width,initial-scale=1'><title>Micro Lifecycle Authorized</title></head><body style='font-family:system-ui;background:#050a11;color:#eef7ff;padding:20px'><h2>ONE MICRO LIFECYCLE VALIDATION AUTHORIZED</h2><p>Exactly one qualifying entry attempt is authorized.</p><p>NO_FILL consumes the attempt. A fill remains managed until governed exit/flat resolution.</p><p><b>SECOND AUTHORIZATION: DISABLED</b></p></body></html>",{status:200,headers:{...HTML_HEADERS,"cache-control":"no-store","x-nfe-control":"MICRO_LIFECYCLE_ONE_SHOT_AUTHORIZED"}});
    }
...
document.getElementById('export')?.addEventListener('click',async()=>{const r=await fetch('/founder-execution-proof-export',{cache:'no-store'});if(!r.ok){alert('EXPORT FAILED: '+r.status);return;}const blob=await r.blob();const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='baseline-real-execution-proof-'+new Date().toISOString().replace(/[:.]/g,'-')+'.json';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(a.href),1000);});document.getElementById('rearm')?.addEventListener('click',async()=>{if(!confirm('Rearm exactly ONE manual-only $1-max execution proof? This does not submit an order.'))return;const r=await fetch('/founder-rearm-execution-proof',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({authorization:'REARM_ONE_EXECUTION_PROOF_MAX_1_USD'})});const j=await r.json();alert(j.ok?'REARMED: one manual-only $1 execution proof is authorized. No order has been submitted.':'NOT REARMED: '+(j.state||r.status));location.reload();});document.getElementById('run')?.addEventListener('click',async()=>{if(!confirm('Run exactly ONE $1-max execution proof now? This can place real Kalshi orders.'))return;const b=document.getElementById('run'),o=document.getElementById('result');b.disabled=true;b.textContent='RUNNING…';o.textContent='Submitting governed execution proof…';try{const r=await fetch('/founder-run-qualified-trade-now',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({authorization:'FOUNDER_RUN_QUALIFYING_ONE_TRADE_NOW'})});const j=await r.json();o.textContent=JSON.stringify(j,null,2);b.textContent='COMPLETE — REVIEW EVIDENCE';}catch(e){o.textContent='REQUEST FAILED: '+String(e);b.textContent='FAILED — NO RETRY';}});</script></body></html>`,{headers:{"content-type":"text/html; charset=utf-8","cache-control":"no-store"}});
    ...
    if (request.method === "GET" && url.pathname === "/founder-manual-close-preview") {
      const recordId=String(url.searchParams.get("recordId")||"").trim(),requestedCount=Number(url.searchParams.get("count"));
      const ledger=await loadFounderManualLedger(env),records=Array.isArray(ledger?.records)?ledger.records:[];
      const record=records.find(r=>String(r?.id||r?.recordId||r?.entryOrderId||"")===recordId)||null;
      if(!record)return json({ok:false,state:"FOUNDER_CLOSE_REJECT_POSITION_NOT_FOUND_OR_UNOWNED",source:FOUNDER_MANUAL_SOURCE,providerWrites:0,submitted:false,manualCloseAuthorized:false},404);
      const result=await founderManualClosePreview(env,record,requestedCount);
      return json(result,result.ok?200:409);
    }

    if (request.method === "GET" && url.pathname === "/founder-manual-close-proof") {
      const founder={id:"MOCK-FOUNDER-1",source:FOUNDER_MANUAL_SOURCE,ticker:"KXMOCK15M-TEST",outcomeSide:"YES",entryOrderId:"MOCK-ENTRY",entryFillQuantity:3,closeFillQuantity:1,status:"OPEN"};
      const auto={...founder,id:"MOCK-AUTO-1",source:AUTO_BASELINE_SOURCE};
      const valid=founderManualCloseDryRun(founder,{requestedCount:2,liveOutcomeBid:.42,quoteAgeMs:100,providerQuantity:2});
      const autoRejected=founderManualCloseDryRun(auto,{requestedCount:2,liveOutcomeBid:.42,quoteAgeMs:100,providerQuantity:2});
