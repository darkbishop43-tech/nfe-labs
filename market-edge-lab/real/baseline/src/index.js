...
async function load(){
  ...
  try{
    ...
    creds.className=creds.textContent==='INSTALLED'?'good':'bad';gatePreview.textContent=preview?.ok&&preview?.submitted===false?'PASS · NO SUBMISSION':((preview?.diagnostic?.category||preview?.state||'NOT PROVEN')+(preview?.diagnostic?.httpStatus?' · HTTP '+preview.diagnostic.httpStatus:'')+(!preview?.ok&&preview?.discovery?' · E:'+String(preview.discovery.eventsScanned??'?')+' C:'+String(preview.discovery.cryptoEvents??'?')+' S:'+String(preview.discovery.eventSlugCount??'?')+' M:'+String(preview.discovery.marketsReturned??'?')+' O:'+String(preview.discovery.activeOpenMarkets??'?')+' L:'+String(preview.discovery.positiveLiquidityMarkets??'?'):''));gatePreview.className=preview?.ok&&preview?.submitted===false?'good':'m';
