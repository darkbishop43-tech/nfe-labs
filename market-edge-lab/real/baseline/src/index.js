  ...
  try{
    ...
    if(testPositionSummary&&testPositionGrid){
      ...
      if(openTestPositions.length){
        ...
        testPositionGrid.innerHTML=recentAttempts.length?recentAttempts.map(a=>'<div class="opp"><div class="oppHead"><div class="q">Attempt '+Number(a?.attemptNo||0)+' · '+esc(a?.asset||'')+' · '+esc(a?.side||'')+'</div><div class="tag">'+esc(a?.status||'UNKNOWN')+'</div></div><div class="meta">'+esc(a?.ticker||'')+' · score '+(Number.isFinite(Number(a?.liveScore))?Number(a.liveScore).toFixed(2):'—')+(a?.orderId?' · order '+esc(a.orderId):'')+'</div></div>').join(''):'<div class="opp"><div class="meta">No execution-test position is currently open.</div></div>';
      }
    }
