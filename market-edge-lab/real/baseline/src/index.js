...
<div class="w">
  ...
  <div class="card section">
    ...
    <div class="focusChart" aria-live="polite">
      <div class="focusChartTop"><div><div class="label">SELECTED 24-HOUR CHART</div><div id="focusChartAsset" class="q">BITCOIN · BTC</div></div><div><div id="focusChartPrice" class="focusChartPrice">CHECKING…</div><div id="focusChartChange" class="marketChange">—</div></div></div>
      <svg id="focusChartSvg" viewBox="0 0 100 40" preserveAspectRatio="none" aria-label="Selected asset 24-hour price chart"></svg>
    </div>
...
async function load(){
  ...
  try{
    ...
    const drawFocusChart=(asset)=>{
      ...
      const vals=(Array.isArray(p.points)?p.points:[]).map(x=>Number(x.price)).filter(Number.isFinite);
      E('focusChartAsset').textContent=chartNames[asset]||asset.toUpperCase();
      E('focusChartPrice').textContent=Number.isFinite(Number(p.current))?moneyFmt(Number(p.current)):'UNAVAILABLE';
      ...
      if(vals.length<2){svg.innerHTML='';return;}
      const lo=Math.min(...vals),hi=Math.max(...vals),span=(hi-lo)||1;
      const coords=vals.map((v,i)=>((i/(vals.length-1))*100).toFixed(2)+','+(38-((v-lo)/span)*36).toFixed(2)).join(' ');
      svg.className=Number(p.changePct)>=0?'good':'bad';
      svg.innerHTML='<line class="base" x1="0" y1="38" x2="100" y2="38"></line><polyline points="'+coords+'"></polyline>';
    };
