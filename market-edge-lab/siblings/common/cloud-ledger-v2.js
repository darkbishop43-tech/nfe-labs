// Sibling cloud ledger reader V2 — authoritative display state, PAPER ONLY.
// Reads only the isolated sibling Worker. It never executes trades in the browser.
(() => {
  ...
  function paint(state) {
    // Keep dashboard.js's in-memory state aligned when the shared dashboard exposes it
    // as the classic-script lexical binding. Failure here does not affect the direct DOM proof below.
    ...
    if (pnl) {
      pnl.textContent = (Number(state.realizedPnl) < 0 ? '-' : '') + money(Math.abs(Number(state.realizedPnl || 0)));
      pnl.className = 'val ' + (Number(state.realizedPnl) < 0 ? 'bad' : Number(state.realizedPnl) > 0 ? 'good' : '');
    }
  ...
  async function sync() {
    try {
      ...
      console.error('Sibling cloud ledger V2 sync failed:', err);
    }
  }
